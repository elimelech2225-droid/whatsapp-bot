...
כדי לראות משלוחים שמתאימים לאזור שבו אתה פנוי, פשוט שלח:

*פ ירושלים*
או
*פנוי ירושלים*

ניתן לרשום כל עיר אחרת, לדוגמה:
*פ תל אביב*
*פ ראשון לציון*
*פ חיפה*

המערכת תציג לך את המשלוחים הפעילים והרלוונטיים לאזור שבחרת.

📦 *מצאת משלוח שמתאים לך?*

לחץ על *„אני מעוניין”* וציין תוך כמה דקות אתה יכול להגיע לנקודת האיסוף.

הבקשה שלך תועבר לסדרן. אם הסדרן יבחר בך, תקבל הודעה שהמשלוח הוקצה לך ושהסדרן יעביר לך את פרטי המשלוח בפרטי.

⚠️ *חשוב מאוד – לפני שאתה מתחייב למשלוח*

יש ללחוץ „אני מעוניין” ולהתחייב למשלוח רק כאשר אתה בטוח שאתה יכול לבצע אותו ולעמוד בזמן ההגעה שציינת.

לאחר שסגרת את המשלוח מול הסדרן, אין לבטל ללא סיבה מוצדקת.

ביטול לאחר התחייבות, אי-הגעה לאיסוף, מסירת זמן הגעה שאינך יכול לעמוד בו או התנהלות שפוגעת בביצוע המשלוח עלולים לפגוע בסדרן, בלקוח ובשליחים אחרים.

במקרה חריג שבו אינך יכול לבצע משלוח שכבר קיבלת, יש לעדכן את הסדרן בהקדם האפשרי ולא להיעלם או להשאיר את המשלוח ללא טיפול.

❗ *שימוש לא תקין במערכת*

ביטולים חוזרים לאחר התחייבות, אי-הגעה למשלוחים או התנהלות שאינה עומדת בכללי הפלטפורמה עשויים להביא להגבלת החשבון, להשעייתו או להסרתו מהפלטפורמה, בהתאם לנסיבות.

✅ בקש משלוח רק כשאתה באמת זמין.
✅ ציין זמן הגעה אמיתי.
✅ לאחר שסגרת עם הסדרן – עמוד בהתחייבות שלך.
...
def notify_admin_new_registration(user):
    if not user:
        print("ADMIN REGISTRATION NOTICE ERROR: user is None")
        return False
...
ניתן להתחיל מחדש בכל עת."""
    ...
    if action_id == "agreement_accept":
        ...
        if not user:
            send_message(
                phone,
                "אירעה תקלה ביצירת החשבון. נסה שוב בעוד רגע."
            )
            return True
...
12"""
        ...
        if state == "customer_cancel_id":
            ...
            if (
                not shipment
                or shipment["customer_id"] != user["id"]
            ):
                send_message(
                    phone,
                    "המשלוח לא נמצא."
...
המשלוח אינו מבוטל עד שהמנהל מאשר את הבקשה."""
        ...
        if action_id == "customer_subscription":
            subscription_menu(
                phone,
                user
            )
...
{areas}"""
    ...
    if match and user["role"] == ROLE_DRIVER:
        shipment_id = int(
            match.group(1)
        )
